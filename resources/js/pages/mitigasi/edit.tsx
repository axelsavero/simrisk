...
import { BreadcrumbItem, IdentifyRisk, Mitigasi } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Mitigasi', href: '#' },
];

export default function Edit() {
    const { mitigasi, identifyRisks, statusOptions, strategiOptions } = usePage<PageProps>().props;
    const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
    
    const { data, setData, post, processing, errors } = useForm<FormData>({
        identify_risk_id: mitigasi.identify_risk_id.toString(),
        ...
        biaya_mitigasi: mitigasi.biaya_mitigasi?.toString() || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post(`/mitigasi/${mitigasi.id}`, formData, {
            onSuccess: () => {
                Swal.fire({
                    title: 'Berhasil!',
                    text: 'Mitigasi berhasil diperbarui.',
                    icon: 'success',
                    confirmButtonText: 'OK'
                }).then(() => {
                    router.visit(`/mitigasi/${mitigasi.id}`);
                });
            },
            onError: () => {
                Swal.fire({
                    title: 'Error!',
                    text: 'Gagal memperbarui mitigasi. Periksa kembali data yang dimasukkan.',
                    icon: 'error',
                    confirmButtonText: 'OK'
                });
            }
    ...
    const handleDownloadExistingFile = (filename: string) => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Mitigasi - ${mitigasi.judul_mitigasi}`} />
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Edit Mitigasi</h1>
                            <p className="mt-1 text-sm text-gray-600">
                                Perbarui rencana mitigasi untuk risiko {mitigasi.identify_risk?.id_identify}
                            </p>
                        </div>
                        <Link
                        ...
                        <div className="space-y-6">
                                        ...
                                        {mitigasi.bukti_implementasi.map((file, index) => (
                                            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <button
                                                        type="button"
                                                        onClick={() => handleDownloadExistingFile(file.filename)}
                            ...
                            <div>
                                ...
                                {selectedFiles.length > 0 && (
                                    <div className="mt-4">
                                        <h4 className="text-sm font-medium text-gray-700 mb-2">File baru yang dipilih:</h4>
                                        <div className="space-y-2">
